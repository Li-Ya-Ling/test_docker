...
write_matrix_with_out_row_col_name = function( mat, out_file ){
    write.table(as.data.frame(mat),sep="\t",quote=FALSE, file=out_file, row.names = FALSE, col.names = FALSE)
}
lo = function(rown, coln, nrow, ncol, cellheight = NA, cellwidth = NA, treeheight_col, treeheight_row, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, gaps_row, gaps_col, ...){
    ...
    realtreeheight_row = convertWidth(treeheight_row,"bigpts",valueOnly = T)
    realtreeheight_col = convertWidth(treeheight_col,"bigpts",valueOnly = T)
    ...
    res = list(gt = gt, mindim = mindim, realtreeheight_row = realtreeheight_row, realtreeheight_col = realtreeheight_col, cw = cw, ch = ch)
...
heatmap_motor = function(matrix, out_file = NA, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, number_color, gaps_col, gaps_row, labels_row, labels_col, ...){
    ...
    res = lo$gt
    mindim = lo$mindim
    realtreeheight_row = lo$realtreeheight_row
    realtreeheight_col = lo$realtreeheight_col
    cw = lo$cw
    ch = lo$ch
    
    # Output clustered matrix and coordinate
    if(is.na(filename) && !is.na(out_file) ){
        coordinate1 = c("treeheight_row","treeheight_col","cellwidth","cellheight")
        coordinate2 = c(realtreeheight_row,realtreeheight_col,cw,ch)
        coordinate = data.frame(coordinate1,coordinate2)
        print(coordinate)
        coordinate_file = paste(out_file,".coordinate",sep="")
        write_matrix_with_out_row_col_name( coordinate, coordinate_file )
        
#        write_matrix_with_out_row_col_name( res, gt_file )
#        cat(res,file="gt_file")
...
pheatmapm = function(mat, out_file = NA, color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100), kmeans_k = NA, breaks = NA, border_color = "grey60", cellwidth = NA, cellheight = NA, scale = "none", cluster_rows = TRUE, cluster_cols = TRUE, clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "complete", clustering_callback = identity2, cutree_rows = NA, cutree_cols = NA,  treeheight_row = ifelse((class(cluster_rows) == "hclust") || cluster_rows, 50, 0), treeheight_col = ifelse((class(cluster_cols) == "hclust") || cluster_cols, 50, 0), legend = TRUE, legend_breaks = NA, legend_labels = NA, annotation_row = NA, annotation_col = NA, annotation = NA, annotation_colors = NA, annotation_legend = TRUE, annotation_names_row = TRUE, annotation_names_col = TRUE, drop_levels = TRUE, show_rownames = T, show_colnames = T, main = NA, fontsize = 10, fontsize_row = fontsize, fontsize_col = fontsize, display_numbers = F, number_format = "%.2f", number_color = "grey30", fontsize_number = 0.8 * fontsize, gaps_row = NULL, gaps_col = NULL, labels_row = NULL, labels_col = NULL, filename = NA, width = NA, height = NA, silent = FALSE, ...){
    ...
    if( !is.na(out_file) ){
	out_file_after_clust = paste(out_file,".BeforeClust",sep='')
        write_matrix( mat, out_file_after_clust )
    }
    print("aaa")
    print(mat)
    ...
    if((class(cluster_rows) == "hclust") || cluster_rows){
        if(class(cluster_rows) == "hclust"){
            ...
            print("merge")
            print(tree_row$merge)
            print("height")
            print(tree_row$height)
            print("labels")
            print(tree_row$labels)
            print("method")
            print(tree_row$method)
            print("call")
            print(tree_row$call)
            print("dist.method")
            print(tree_row$dist.method)
        }
    ...
    #find best K
    if( !is.na(out_file) ){
        mat_new = as.data.frame(mat)
        if (length(mat_new[,1]) <= 100) {
            max_nc = length(mat_new[,1]) - 1
        } else {
            max_nc = 100
        }
        yy=NbClust(mat, distance = clustering_distance_rows, min.nc=2, max.nc=max_nc, method = clustering_method, index = "silhouette", alphaBeale = 0.1)
        print(yy$Best.nc)
        k=yy$Best.nc[[1]]
        hc_genes <- agnes(mat, diss=FALSE, metric = clustering_distance_rows, method = clustering_method) # cluster genes
        mycl <- cutree(as.hclust(hc_genes), k=k)
        for (i in 1:k){
            subcluster_i=mat_new[mycl==i,]
            fname=paste(out_file,'_subcluster_', i, '.data', sep='')
            write.table(subcluster_i, file=fname, quote=F, sep='\t', col.names=T)
            #plot the cluster i
            ymin=min(subcluster_i);ymax=max(subcluster_i);
            class_name= paste('subcluster_', i, sep='')
            #plot.new()
            plot_label=paste(class_name,', ', length(subcluster_i[,1]), " genes", sep='')
            plot(as.numeric(subcluster_i[1,]), type='l', ylim=c(0.7, ymax), main=plot_label, col='lightgray', xaxt='n', xlab='', ylab='RPKM')
            axis(side=1, at=1:length(subcluster_i[1,]), labels=colnames(subcluster_i), las=2)
            #x=c(1,2)
    ...
    # Output clustered matrix color
    if( !is.na(out_file) ){
        color_file = paste(out_file,".color",sep="")
        write_matrix( mat, color_file )
        for (i in 1:k){
            subcluster_i=mat_for_div[mycl==i,]
            subcluster_i = scale_colours(subcluster_i, col = color, breaks = breaks)
            div_heatmap_fname=paste(out_file,'_subcluster_', i, '.png', sep='')
            div_height = 3.5
            gt = heatmap_motor(subcluster_i, border_color = border_color, cellwidth = cellwidth, cellheight = cellheight, treeheight_col = 0, treeheight_row = 0, tree_col = NA, tree_row = NA, filename = div_heatmap_fname, width = width, height = div_height, breaks = breaks, color = color, legend = legend, annotation_row = NA, annotation_col = NA, annotation_colors = NA, annotation_legend = annotation_legend, annotation_names_row = annotation_names_row, annotation_names_col = annotation_names_col, main = paste("Group ", i, sep=""), fontsize = fontsize, fontsize_row = fontsize_row, fontsize_col = fontsize_col, fmat = fmat, fontsize_number = fontsize_number, number_color = number_color, gaps_row = gaps_row, gaps_col = gaps_col, labels_row = NULL, labels_col = NULL, ...)
        }
    }
...
--border_color		character	color of border, [optional, default grey60]
\n")
	q(status=1);
}
...
if ( is.null(opt$border_color))		{ opt$border_color = NA }
...
# color
# check
if ( (opt$color.type < 1) || (opt$color.type > 7) ) {
	cat("Final Error: color.type must be 1-7\n")
	print_usage(spec)
}
# set
...
color.7 <- colorRampPalette(rev(c("#ff0000", "#d9d9d9",  "#0000ff")))(100)
color.set <- list(color.1, color.2, color.3, color.4, color.5, color.6, color.7)

# output pdf
pheatmapm(data, border_color = opt$border_color, scale=opt$scale, height=height, width=width, color=color.set[[opt$color.type]], 
